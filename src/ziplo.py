...
class ZiploApp(tk.Tk):
    ...
    def _write_workflow_to_repo(self, repo_path):
        ...
        Also removes every OTHER .yml in .github/workflows/ so stale workflows
        (e.g. old conda/environment.yml-based ones) cannot trigger on Actions.
        """
        btype = self.cfg.get("build_config", {}).get("build_type", "none")
        wf_dir = Path(repo_path) / ".github" / "workflows"
        try:
            wf_dir.mkdir(parents=True, exist_ok=True)
            # Remove every existing workflow file so nothing stale survives
            for old_wf in wf_dir.glob("*.yml"):
                old_wf.unlink(missing_ok=True)
            for old_wf in wf_dir.glob("*.yaml"):
                old_wf.unlink(missing_ok=True)
            if btype == "none":
                return True, ""
            yaml = self._generate_workflow_yaml()
            (wf_dir / "build.yml").write_text(yaml, encoding="utf-8")
            return True, ""
        except Exception as e:
            return False, str(e)
