...
class ZiploApp(tk.Tk):
    ...
    def _page_accounts(self, parent):
        ...
        link.bind("<Button-1>", lambda e: webbrowser.open(
            "https://github.com/settings/tokens/new?scopes=repo&description=Ziplo"))
        tk.Label(add_inner, text="Required scopes: contents (read & write), metadata (read)",
    ...
    def _page_build(self, parent):
        ...
        p = inner
        tk.Label(p, text="Build Setup", font=F["display"],
                 fg=C["text"], bg=C["bg"]).pack(anchor="w", padx=20, pady=(20, 4))
        tk.Label(p, text="Configure how your project compiles before shipping. "
                         "Ziplo runs these commands inside the extracted folder before git add.",
                 font=F["small"], fg=C["text_dim"], bg=C["bg"],
                 wraplength=700, justify="left").pack(anchor="w", padx=20, pady=(0, 16))

        # ── Electron ──────────────────────────────────────────────────────
        self._build_section(p, "Electron",
            "Build an Electron app into a distributable before releasing.",
            [
                ("electron_enabled",  "Enable Electron build",  True,  "check"),
                ("electron_npm_i",    "Run npm install first",  True,  "check"),
                ("electron_cmd",      "Build command",          "npm run build",    "entry"),
                ("electron_dist_cmd", "Package command",        "npm run dist",     "entry"),
                ("electron_out_dir",  "Output folder",         "dist",             "entry"),
            ],
            tip='Typical setup: build command = "npm run build", package = "npm run dist". Make sure package.json has an "electron-builder" config.',
            link=("electron-builder docs", "https://www.electron.build/")
        )

        # ── Python / PyInstaller ───────────────────────────────────────────
        self._build_section(p, "Python / PyInstaller",
            "Compile a Python app into a .exe before releasing.",
            [
                ("pyinst_enabled",    "Enable PyInstaller build",        True,   "check"),
                ("pyinst_venv",       "Create virtualenv first",         False,  "check"),
                ("pyinst_req",        "Install requirements.txt",        True,   "check"),
                ("pyinst_entry",      "Entry script",                    "main.py", "entry"),
                ("pyinst_extra_args", "Extra PyInstaller args",          "--onefile --windowed", "entry"),
                ("pyinst_out_dir",    "Output folder",                   "dist", "entry"),
            ],
            tip='Ziplo runs: pip install pyinstaller, pip install -r requirements.txt (if enabled), then pyinstaller with your args and entry script.',
            link=("PyInstaller docs", "https://pyinstaller.org/")
        )

        # ── Custom / Generic ──────────────────────────────────────────────
        self._build_section(p, "Custom build command",
            "Run any shell command before committing. Use this for any other build system.",
            [
                ("custom_enabled", "Enable custom build", False, "check"),
                ("custom_cmd",     "Command to run",      "",    "entry"),
                ("custom_cwd",     "Working directory",   "",    "entry"),
            ],
            tip='Runs inside the extracted folder unless you set a working directory. Examples: make release, cargo build --release, gradlew assembleRelease',
        )

        # Save button
        save_f = tk.Frame(p, bg=C["bg"])
        save_f.pack(fill="x", padx=20, pady=(8, 24))
        tk.Button(save_f, text="Save build config", font=F["body_b"],
                  bg=C["navy"], fg=C["mint"],
                  activebackground=C["surface3"], activeforeground=C["mint"],
    ...
    def _build_section(self, parent, title, subtitle, fields, tip="", link=None):
        tk.Label(parent, text=title, font=F["heading"],
                 fg=C["text"], bg=C["bg"]).pack(anchor="w", padx=20, pady=(12, 2))
        tk.Label(parent, text=subtitle, font=F["small"],
                 fg=C["text_dim"], bg=C["bg"],
                 wraplength=700, justify="left").pack(anchor="w", padx=20, pady=(0, 6))

        card_f = tk.Frame(parent, bg=C["surface"],
                          highlightbackground=C["border"], highlightthickness=1)
        card_f.pack(fill="x", padx=14, pady=(0, 4))
        card_inner = tk.Frame(card_f, bg=C["surface"])
        card_inner.pack(fill="x", padx=14, pady=12)

        for key, label_text, default, kind in fields:
            row = tk.Frame(card_inner, bg=C["surface"])
            row.pack(fill="x", pady=3)
            ...
            if kind == "check":
                ...
                cb = tk.Checkbutton(row, text=label_text, variable=var,
                                    ...
                                    relief="flat", cursor="hand2")
                cb.pack(anchor="w")
                setattr(self, f"bv_{key}", var)
            else:
                tk.Label(row, text=label_text, font=F["label"],
                         fg=C["text_dim"], bg=C["surface"],
                ...
                e = tk.Entry(row, textvariable=var,
                             ...
                             highlightthickness=1,
                             highlightbackground=C["border"],
                             highlightcolor=C["accent"], width=44)
                e.pack(side="left", padx=(6, 0), ipady=3, ipadx=4)
                setattr(self, f"sv_{key}", var)

        if tip:
            tip_f = tk.Frame(card_inner, bg=C["surface2"],
                             highlightbackground=C["border"], highlightthickness=1)
            tip_f.pack(fill="x", pady=(8, 0))
            tk.Label(tip_f, text=tip, font=F["mono_sm"],
                     fg=C["text_dim"], bg=C["surface2"],
                     justify="left", anchor="w", padx=10, pady=6).pack(anchor="w")

        if link:
            lbl_text, url = link
            lnk = tk.Label(card_inner, text=f"  → {lbl_text}",
    ...
    def _save_build_config(self):
        ...
        self.after(2000, lambda: self.build_save_lbl.configure(text=""))
    ...
    def _load_build_config(self):
        ...
        for key, val in bc.items():
            attr_b = f"bv_{key}"
            attr_s = f"sv_{key}"
            if hasattr(self, attr_b):
                try: getattr(self, attr_b).set(val)
                except Exception: pass
            if hasattr(self, attr_s):
                try: getattr(self, attr_s).set(val)
                except Exception: pass

    def _run_build_steps(self, repo_path, log_cb):
        """Run any configured build steps. Returns (ok, error_msg)."""
        bc = self.cfg.get("build_config", {})

        # ── Electron ────────────────────────────────────────────────────
        if bc.get("electron_enabled"):
            log_cb("\nElectron build…", "heading")
            if bc.get("electron_npm_i"):
                ok, out = _run_cmd(["npm", "install"], repo_path, log_cb)
                if not ok:
                    return False, f"npm install failed: {out}"
            build_cmd = bc.get("sv_electron_cmd", "npm run build").split()
            ok, out = _run_cmd(build_cmd, repo_path, log_cb)
            if not ok:
                return False, f"Electron build failed: {out}"
            dist_cmd = bc.get("sv_electron_dist_cmd", "npm run dist").split()
            ok, out = _run_cmd(dist_cmd, repo_path, log_cb)
            if not ok:
                return False, f"Electron package failed: {out}"

        # ── PyInstaller ─────────────────────────────────────────────────
        if bc.get("pyinst_enabled"):
            log_cb("\nPyInstaller build…", "heading")
            ok, out = _run_cmd(["pip", "install", "pyinstaller"], repo_path, log_cb)
            if not ok:
                return False, f"pip install pyinstaller failed: {out}"
            if bc.get("pyinst_req"):
                ...
                if req.exists():
                    ok, out = _run_cmd(
                        ...
                        repo_path, log_cb)
                    if not ok:
                        return False, f"pip install requirements failed: {out}"
            entry = bc.get("sv_pyinst_entry", "main.py")
            extra = bc.get("sv_pyinst_extra_args", "--onefile --windowed").split()
            cmd = ["pyinstaller"] + extra + [entry]
            ok, out = _run_cmd(cmd, repo_path, log_cb)
            if not ok:
                return False, f"PyInstaller failed: {out}"

        # ── Custom ──────────────────────────────────────────────────────
        if bc.get("custom_enabled"):
            log_cb("\nCustom build…", "heading")
            cmd_str = bc.get("sv_custom_cmd", "").strip()
            if cmd_str:
                cwd = bc.get("sv_custom_cwd", "").strip() or repo_path
                import shlex
                ok, out = _run_cmd(shlex.split(cmd_str), cwd, log_cb)
                if not ok:
                    return False, f"Custom build failed: {out}"
    ...
    def _upload_worker(self, opts):
        ...
        try:
            ...
            if not ok: raise RuntimeError(f"git push failed: {out}")
            if self._cancelled: raise RuntimeError("Cancelled")
