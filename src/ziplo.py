...
C = {
    ...
    # Nav active pill — solid approximation of indigo at low opacity
    "pill":         "#1e1b3a",
    "pill_border":  "#4a3fb5",
}

F = {
    "logo":    ("Segoe UI Variable", 16, "bold"),
    "display": ("Segoe UI Variable", 14, "bold"),
    "heading": ("Segoe UI Variable", 10, "bold"),
    "label":   ("Segoe UI Variable", 9),
    "label_b": ("Segoe UI Variable", 9, "bold"),
    "body":    ("Segoe UI Variable", 10),
    "body_b":  ("Segoe UI Variable", 10, "bold"),
...
def autodetect_project(repo_path):
    ...
    if "package.json" in files:
        ...
        if "electron" in deps or "electron-builder" in deps or "electron" in str(pkg).lower():
            # Pick build command — but skip if it IS electron-builder
            # (running it would trigger publish before --publish never takes effect)
            build_cmd = ""
            for bc in ["build", "compile", "webpack", "vite:build"]:
                if bc in scripts:
                    candidate = scripts[bc]
                    if "electron-builder" not in candidate.lower():
                        build_cmd = f"npm run {bc}"
                    break
            # Pick dist/package command
            for dc in ["dist", "package", "make", "pack", "electron:build"]:
                if dc in scripts:
                    dist_cmd = f"npm run {dc}"
                    break
            else:
                dist_cmd = "npm run dist"
            reason = f"Found package.json with electron"
            if build_cmd:
                reason += f" (build={build_cmd}, dist={dist_cmd})"
            else:
                build_cmd = "npm run build"
                reason += f" (guessed build={build_cmd}, dist={dist_cmd})"
            return {
                "type":      "electron",
                "build_cmd": build_cmd,
                "dist_cmd":  dist_cmd,
...
class ZiploApp(tk.Tk):
    ...
    def _build_ui(self):
        # Sidebar 190px fixed
        self.sidebar = tk.Frame(self, bg=C["surface"], width=200)
        ...
        # Topbar
        topbar = tk.Frame(self.right, bg=C["surface"], height=48)
        topbar.pack(fill="x")
    ...
    def _nav_btn(self, parent, text, key, page_title):
        ...
        def _set_active():
            f.configure(bg=C["pill"], highlightbackground=C["pill_border"],
                        highlightthickness=1)
            lbl.configure(bg=C["pill"], fg=C["accent_hover"])
        def _set_idle():
            f.configure(bg=C["surface"], highlightthickness=0)
            lbl.configure(bg=C["surface"], fg=C["text_dim"])
        def _set_hover():
            f.configure(bg=C["surface2"], highlightthickness=0)
            lbl.configure(bg=C["surface2"], fg=C["text"])
    ...
    def _build_dropzone(self, parent):
        tk.Label(parent, text="Source", font=F["heading"],
        ...
        # Status label
        tk.Label(inner, text="Select a zip archive or a project folder",
                 font=F["body_b"], fg=C["text"],
                 bg=C["surface2"]).pack(anchor="w")

        self.archive_var = tk.StringVar(value="No file or folder selected")
        self.archive_lbl = tk.Label(inner, textvariable=self.archive_var,
                                    font=F["mono_sm"], fg=C["text_muted"],
                                    bg=C["surface2"])
        self.archive_lbl.pack(anchor="w", pady=(2, 8))

        # Both browse buttons side by side on one row
        btn_row = tk.Frame(inner, bg=C["surface2"])
        btn_row.pack(anchor="w")

        tk.Button(btn_row, text="Browse Archive...", font=F["small"],
                  bg=C["surface3"], fg=C["accent"],
                  activebackground=C["border"], activeforeground=C["accent_hover"],
                  relief="flat", padx=12, pady=5, cursor="hand2",
                  command=self._browse_archive).pack(side="left")

        tk.Button(btn_row, text="Browse Folder...", font=F["small"],
                  bg=C["surface3"], fg=C["accent"],
                  activebackground=C["border"], activeforeground=C["accent_hover"],
                  relief="flat", padx=12, pady=5, cursor="hand2",
                  command=self._browse_source_folder).pack(side="left", padx=(8, 0))
    ...
    def _page_build(self, parent):
        ...
        self.el_node_var      = tk.StringVar(value="22")
    ...
    def _generate_workflow_yaml(self):
        ...
        if btype == "pyinstaller":
            ...
            # PowerShell script to delete existing release — using single quotes
            # inside the list strings to avoid Python/YAML quote conflicts
            ps_delete = (
                ...
                "            Write-Host \"Deleting release $($rel.id) for tag $tag\"\n"
                "            Invoke-RestMethod -Method Delete -Uri \"https://api.github.com/repos/$repo/releases/$($rel.id)\" -Headers $headers\n"
                "          } catch {\n"
                "            Write-Host \"No existing release for tag $tag — clean slate\"\n"
                "          }\n"
            )
        ...
        for ref in refs_to_check:
            ...
            for item in data:
                ...
                if not (fname.endswith(".yml") or fname.endswith(".yaml")):
                    continue
                # Only keep ziplo-build.yml — but still delete old versions
                # by checking if the content has changed. Simplest: delete all
                # and let Ziplo rewrite from scratch on this push.
                # This ensures stale "cache: npm" / "node: 20" configs are gone.
                if fname == "ziplo-build.yml":
                    # Delete it too so the fresh one from this push takes effect
                    sha  = item.get("sha", "")
                    path = item.get("path", "")
                    log_cb(f"  Refreshing ziplo-build.yml on remote…", "dim")
                    github_api("DELETE",
                        f"/repos/{repo}/contents/{path}",
                        token,
                        {
                            "message": "ci: refresh ziplo-build.yml [ziplo]",
                            "sha": sha,
                            "branch": ref,
                        })
                    continue
    ...
    def _load_build_config(self):
        bc = self.cfg.get("build_config", {})
        # Upgrade stale node version
        if bc.get("sv_el_node_var") == "20":
            bc["sv_el_node_var"] = "22"
            self.cfg["build_config"] = bc
        if hasattr(self, "build_type_var"):
    ...
    def _browse_source_folder(self):
        path = filedialog.askdirectory(title="Select project folder to upload")
        if path:
            self.archive_var.set(f"📁  {Path(path).name}")
            self._full_archive_path = path
            self._source_is_folder = True
            self.archive_lbl.configure(fg=C["accent"])
            if not self.message_var.get():
                self.message_var.set(f"Release: {Path(path).name}")
    ...
    def _start_upload(self):
        ...
        opts = {
            ...
            "source_is_folder": getattr(self, "_source_is_folder", False),
        }
    ...
    def _upload_worker(self, opts):
        ...
        try:
            ...
            # Step 0 — Extract or use folder directly
            step(0, 5, "Preparing source…")
            source_is_folder = opts.get("source_is_folder", False)

            if source_is_folder:
                # User selected a folder — use it directly, no extraction needed
                repo_path = opts["archive"]
                self._log(f"Using folder: {repo_path}", "dim")
                if not Path(repo_path).is_dir():
                    raise RuntimeError(f"Folder not found: {repo_path}")
            else:
                ...
                self._log(f"Extracting archive…", "dim")
            ...
            build_type = self.cfg.get("build_config", {}).get("build_type", "none")
            if hasattr(self, "build_type_var"):
                build_type = self.build_type_var.get() or build_type
            if build_type == "electron":
                # For Electron projects electron-builder handles release creation
                # and upload via --publish always. Do NOT pre-create the release
                # here — electron-builder refuses to upload to a published release
                # that is older than 2 hours.
                self._log("\nElectron build detected — release will be created by\n"
                          "  electron-builder via GitHub Actions (--publish always)", "dim")
            elif opts["create_release"]:
            ...
            if self._temp_dir and not opts["dest_folder"] and not opts.get("source_is_folder"):
