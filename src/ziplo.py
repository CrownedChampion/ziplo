...
class ZiploApp(tk.Tk):
    ...
    def _page_upload(self, parent):
        ...
        # Give right col a fixed width
        right.configure(width=270)
        ...
        self.go_btn.pack(fill="x", pady=(0, 6))
    ...
    def _upload_worker(self, opts):
        ...
        try:
            ...
            # Step 3 — Pull
            step(3, 50, "Pulling latest…")
            if opts["pull_first"]:
                self._log("\nPulling…", "heading")
                ok, out = run_git(["pull", "origin", opts["branch"],
                                   "--no-edit", "--allow-unrelated-histories"],
                                  repo_path, self._log)
                if not ok and "couldn't find remote ref" not in out.lower():
                    self._log(f"  Note: {out}", "warn")
            if self._cancelled: raise RuntimeError("Cancelled")

            # Step 4 — Push
            step(4, 65, "Pushing to GitHub…")
            self._log("\nPushing…", "heading")
            ok, out = run_git(["push", "-u", "origin", opts["branch"]],
                              repo_path, self._log)
