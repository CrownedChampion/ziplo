...
class ZiploApp(tk.Tk):
    ...
    def _page_build(self, parent):
        ...
        self.py_entry_var  = tk.StringVar(value="main.py")
        self.py_args_var   = tk.StringVar(value="--onefile --windowed")
        self.py_name_var   = tk.StringVar(value="app")
    ...
    def _generate_workflow_yaml(self):
        ...
        if btype == "pyinstaller":
            ...
            # Build the YAML as a plain string — no conda, no environment.yml,
            # pure pip so it works on the default windows-latest runner
            return (
            ...
            node_ver  = node_ver.get()  if node_ver  else "20"
            ...
            return "# No build workflow configured\n"
    ...
    def _write_workflow_to_repo(self, repo_path):
        ...
        """
        btype = self.cfg.get("build_config", {}).get("build_type", "none")
        ...
        try:
            ...
            (wf_dir / "build.yml").write_text(yaml_content, encoding="utf-8")
            return True, ""
        except Exception as e:
            return False, str(e)

    def _delete_remote_stale_workflows(self, token, repo, branch, log_cb):
        """Delete any workflow files on the remote that aren't build.yml.
        Called after push so the cleaned state is reflected on GitHub too.
        """
        # List contents of .github/workflows on the remote
        ...
        for item in data:
            fname = item.get("name", "")
            # Keep our build.yml, delete everything else
            if fname != "build.yml" and (fname.endswith(".yml") or fname.endswith(".yaml")):
                sha = item.get("sha", "")
                path = item.get("path", "")
                log_cb(f"  Removing stale remote workflow: {fname}", "warn")
                github_api("DELETE",
                    f"/repos/{repo}/contents/{path}",
                    token,
                    {
                        "message": f"ci: remove stale workflow {fname}",
                        "sha": sha,
                        "branch": branch,
                    })
    ...
    def _load_build_config(self):
        bc = self.cfg.get("build_config", {})
        if hasattr(self, "build_type_var") and "build_type" in bc:
            self.build_type_var.set(bc["build_type"])
    ...
    def _run_build_steps(self, repo_path, log_cb):
        ...
        btype = self.cfg.get("build_config", {}).get("build_type", "none")
        if btype != "none":
            log_cb(f"  ✓ GitHub Actions workflow written → .github/workflows/build.yml", "success")
    ...
    def _upload_worker(self, opts):
        ...
        try:
            ...
            # Step 0b — Write GitHub Actions workflow file into repo
            ...
            # Clean up any stale workflow files on the remote (e.g. old conda ones)
